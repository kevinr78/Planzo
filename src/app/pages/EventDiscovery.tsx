...
function HeroFeaturedCard({ event }: { event: Event }) {
  return (
    <Link to={`/events/${event.id}`} style={{ textDecoration: "none" }}>
      <div
        className="stagger-child relative rounded-3xl overflow-hidden group cursor-pointer"
        style={{ height: 460 }}
      >
        <img
          src={event.image}
...
function SideFeaturedCard({ event }: { event: Event }) {
  return (
    <Link to={`/events/${event.id}`} style={{ textDecoration: "none" }}>
      <div
        className="stagger-child relative rounded-2xl overflow-hidden group cursor-pointer"
        style={{ height: 218 }}
      >
        <img
          src={event.image}
...
export function EventDiscovery() {
  ...
  const mapCenter = useMemo<[number, number]>(() => {
    ...
    const avgLat =
      MOCK_EVENTS.reduce((s, e) => s + e.lat, 0) / MOCK_EVENTS.length;
    const avgLng =
      MOCK_EVENTS.reduce((s, e) => s + e.lng, 0) / MOCK_EVENTS.length;
        ...
        <div ref={mapRef} className="section-reveal py-12">
          <div className="flex items-end justify-between mb-6">
            ...
            >
              ...
              {MOCK_EVENTS.length} events shown
            </div>
          </div>
          <div
